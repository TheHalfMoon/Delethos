...
function applyAmendment013(source) {
  ...
  source = replaceOnce(
    source,
    lines(['    runtime_release: RUNTIME_RELEASE,', '    runtime_commit: RUNTIME_COMMIT,', '    model_revision: MODEL_REVISION,']),
    lines(['    runtime_release: RUNTIME_RELEASE,', '    runtime_commit: RUNTIME_COMMIT,', "    runtime_tag_provenance_transport: 'git-ls-remote-public-no-auth',", "    runtime_release_asset_provenance_transport: 'github-expanded-assets-public-no-auth',", '    model_revision: MODEL_REVISION,']),
  ...
  source = replaceSection(
    ...
    lines([
      ...
      '  const entry = assetEntryForName(html, assetName);', "  const expectedPath = '/ggml-org/llama.cpp/releases/download/' + RUNTIME_RELEASE + '/' + assetName;", "  if (!entry.includes(expectedPath)) throw new Error('runtime release asset entry was not bound to the exact release/tag filename');", '  const digests = [...entry.matchAll(/sha256:([0-9a-fA-F]{64})/g)].map((match) => match[1].toLowerCase());', "  if (digests.length !== 1 || digests[0] !== expectedSha256) throw new Error('runtime release asset public metadata digest did not match the pinned digest');", '  return digests[0];',
      '}', '',
      'async function fetchPublicReleaseAssetDigest(assetName, expectedSha256, timeoutMs = 30_000) {',
      "  const url = 'https://github.com/ggml-org/llama.cpp/releases/expanded_assets/' + RUNTIME_RELEASE;", '  const response = await fetch(url, {', "    redirect: 'follow',", "    headers: { Accept: 'text/html', 'User-Agent': 'delethos-r181' },", '    signal: AbortSignal.timeout(timeoutMs),', '  });', '  const finalURL = new URL(response.url || url);', "  if (finalURL.protocol !== 'https:' || finalURL.hostname !== 'github.com') throw new Error('runtime release metadata redirected outside github.com');", "  if (!response.ok) throw new Error('GET github.com returned HTTP ' + response.status);", '  const text = await response.text();', '  return parseReleaseAssetDigestHtml(text, assetName, expectedSha256);',
      '}', '',
    ]),
  ...
  source = replaceSection(
    ...
    lines([
      'function parsePiToolEvidence(stdout) {', '  const toolCalls = [];', '  const toolResults = [];', '  const starts = [];', '  const ends = [];', '  const assistantIdentities = [];', '  let invalid = false;',
      "  for (const line of stdout.split(/\\r?\\n/).filter((value) => value.trim() !== '')) {", '    let event;', '    try { event = JSON.parse(line); } catch { invalid = true; continue; }', "    if (!event || typeof event !== 'object' || Array.isArray(event)) { invalid = true; continue; }",
      "    if (event.type === 'message_end') {", '      const message = event.message;', "      if (!message || typeof message !== 'object' || Array.isArray(message) || typeof message.role !== 'string') { invalid = true; continue; }",
      "      if (message.role === 'assistant') {", '        assistantIdentities.push({', "          provider: typeof message.provider === 'string' ? message.provider : null,", "          model: typeof message.model === 'string' ? message.model : null,", '        });', '        if (!Array.isArray(message.content)) { invalid = true; continue; }', '        for (const item of message.content) {', "          if (!item || typeof item !== 'object' || Array.isArray(item) || item.type !== 'toolCall') continue;", "          const plainArguments = item.arguments && typeof item.arguments === 'object' && !Array.isArray(item.arguments) && Object.getPrototypeOf(item.arguments) === Object.prototype;", "          if (typeof item.id !== 'string' || item.id.length === 0 || typeof item.name !== 'string' || item.name.length === 0 || !plainArguments) { invalid = true; continue; }", '          toolCalls.push({ id: item.id, name: item.name, arguments: item.arguments });', '        }', '      }',
      "      if (message.role === 'toolResult') {", "        if (typeof message.toolCallId !== 'string' || message.toolCallId.length === 0 || typeof message.toolName !== 'string' || message.toolName.length === 0 || typeof message.isError !== 'boolean') { invalid = true; continue; }", '        toolResults.push({ id: message.toolCallId, name: message.toolName, isError: message.isError });', '      }', '    }',
      "    if (event.type === 'tool_execution_start') {", "      if (typeof event.toolCallId !== 'string' || event.toolCallId.length === 0 || typeof event.toolName !== 'string' || event.toolName.length === 0) invalid = true;", '      else starts.push({ id: event.toolCallId, name: event.toolName });', '    }',
      "    if (event.type === 'tool_execution_end') {", "      if (typeof event.toolCallId !== 'string' || event.toolCallId.length === 0 || typeof event.toolName !== 'string' || event.toolName.length === 0 || typeof event.isError !== 'boolean') invalid = true;", '      else ends.push({ id: event.toolCallId, name: event.toolName, isError: event.isError });', '    }', '  }',
  ...
  source = replaceSection(
    ...
    lines([
      ...
      '  const durableAssistant = {', "    type: 'message_end',", '    message: {', "      role: 'assistant', provider: CANONICAL_PROVIDER, model: CANONICAL_MODEL,", "      content: [{ type: 'toolCall', id: 'call-1', name: 'write', arguments: { path: SMOKE_FILE, content: SMOKE_CONTENT } }],", '    },', '  };',
