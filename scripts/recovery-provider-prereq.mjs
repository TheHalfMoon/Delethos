...
function applyAmendment016(source) {
  ...
  source = replaceOnce(source, "  const baseURL = 'http://127.0.0.1:12345/v1';", lines([
    ...
    "  const witnessStart = { choices: [{ delta: { tool_calls: [{ index: 0, id: 'call-1', type: 'function', function: { name: 'write', arguments: exactWitnessArguments.slice(0, witnessSplit) } }] }, finish_reason: null }] };",
    "  const witnessContinue = { choices: [{ delta: { tool_calls: [{ index: 0, function: { arguments: exactWitnessArguments.slice(witnessSplit) } }] }, finish_reason: null }] };",
    "  const witnessFinish = { choices: [{ delta: {}, finish_reason: 'tool_calls' }] };",
    ...
    "  const plainTextStream = makeSse([{ choices: [{ delta: { content: 'not a tool call' }, finish_reason: 'stop' }] }]);",
    "  const contradictoryFinish = { choices: [{ delta: {}, finish_reason: 'stop' }] };",
