...
import { buildCodexConformanceInvocation, parseCodexJsonl } from '../src/codex.ts';
...
test('Codex conformance invocation is noninteractive JSONL, exact cwd, safe sandbox, and isolated', () => {
  const plan = buildCodexConformanceInvocation(request({ model: 'gpt-test' }), discovery);
...
test('Codex conformance read-only maps to explicit read-only sandbox', () => {
  const plan = buildCodexConformanceInvocation(request({ posture: 'READ_ONLY' }), discovery);
  assert.equal(plan.args[plan.args.indexOf('--sandbox') + 1], 'read-only');
});

test('Codex conformance resume is explicit and non-ephemeral', () => {
  const plan = buildCodexConformanceInvocation(request({ sessionId: 'session-123' }), discovery);
  assert.ok(plan.args.includes('resume'));
  assert.ok(plan.args.includes('session-123'));
  assert.ok(!plan.args.includes('--ephemeral'));
});

test('Codex rejects provider selection even in conformance mode', () => {
  assert.throws(() => buildCodexConformanceInvocation(request({ provider: 'other' }), discovery), /provider selection/);
});

test('Codex JSONL parser captures explicit turn completion, thread, and final agent message', () => {
  const stdout = [
...
test('Codex parser exposes malformed, failed, and missing-completion facts fail-closed', () => {
  ...
  const messageOnly = parseCodexJsonl(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'not enough' } }));
  assert.equal(messageOnly.finalMessage, 'not enough');
  assert.equal(messageOnly.turnCompleted, false);
});
